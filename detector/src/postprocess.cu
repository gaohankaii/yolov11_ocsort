#include "../detector/include/postprocess.h"

// ===== decode kernel: 输出直接为原图坐标 =====
__global__ void decode_yolov11_kernel(
...
// ===== Transpose kernel =====
__global__ void transpose_yolov11_kernel(
    ...
    float iou_thresh, int topk)
{
    ...
    transpose_yolov11_kernel<<<grid, BLOCK>>>(d_output, d_trans, num_classes + 4, num_anchors);
    ...
    decode_yolov11_kernel<<<grid, BLOCK>>>(num_classes, num_anchors, score_thresh,
                                           d_trans, d_boxes, d_count, pparam);
    cudaDeviceSynchronize();
    ...
    // 4. 收集所有 NMS 保留的检测框（先不截断）
    std::vector<Object> all_objs;
    for (int i = 0; i < h_count; ++i) {
        if (h_keep[i] == 0) continue;   // 被 NMS 抑制
        Object obj;
    ...
    size_t keep_num = std::min(static_cast<size_t>(topk), all_objs.size());
    objs.reserve(keep_num);
    for (size_t i = 0; i < keep_num; ++i) {
        objs.push_back(all_objs[i]);
    }
